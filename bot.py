...
MODEL = "gpt-5.6"
BOT_VERSION = "v7.4-forwarded-inbox"
...
def complete_ms_question(chat_id, question_id):
    conn = get_db()
    cur = conn.execute(
        """
        UPDATE ms_questions
        ...
        """,
        (datetime.now(TZ).isoformat(timespec="seconds"), chat_id, question_id),
    )
    ok = cur.rowcount > 0
    conn.commit()
    conn.close()
...
def handle_forwarded_message(chat_id, message):
    ...
    send_message(
        chat_id,
        f"Разбираю пересылку{f' от {source_name}' if source_name else ''}..."
    )
    ...
    try:
        ...
        # Даже если ИИ не разобрал, сам входящий не теряем.
        parsed = {
    ...
    save_forwarded_inbox(chat_id, message, source_name, raw_text, parsed)
...
def build_evening_update(chat_id):
    ...
    q_context = "ВОПРОСЫ / СОГЛАСОВАНИЯ С МС:\n" + (
        "\n".join(f"- Q{q['id']}: {q['text']}" for q in questions)
        if questions else "Нет."
    )

    tasks = get_open_tasks(chat_id)
    task_context = "ОТКРЫТЫЕ ЗАДАЧИ:\n" + (
        "\n".join(f"- #{t['id']}: {t['text']}" for t in tasks[-20:])
        if tasks else "Нет."
    )
    ...
    prompt = f"""
Составь короткий вечерний апдейт для МС, который Маша сможет скопировать и отправить.

Главное — вопросы и решения, которые реально нужны от МС.
Не засоряй апдейт внутренними техническими задачами Маши, если от МС ничего не требуется.

Формат:
Вопросы / нужно решение:
— ...

На контроле / жду:
— ...  (только если МС полезно знать)

Если вопросов нет, прямо скажи это.
Не утверждай, что что-то отправлено или выполнено.
Не добавляй закрытые этапы.

ДАННЫЕ:
{source_context}
"""
    return ask_openai_grounded_brief(chat_id, prompt)
...
def try_handle_command(chat_id, text):
    ...
    if clean in {"/questions","вопросы мс","что спросить у мс"}:
        ...
        return True
    if clean in {"/update","вечерний апдейт","собери вечерний апдейт"}:
    ...
    m = re.match(r"^(?:/answered|ответила мс|мс ответила)\s*#?q?(\d+)", normalized, flags=re.I)
    if m:
        qid = int(m.group(1))
        send_message(
            chat_id,
...
def main():
    init_db()
    print(f"Masha 2.0 {BOT_VERSION} запущена: forwarded inbox + MS questions + evening update + vision.")
    offset = None
    known_chat_ids = set()
