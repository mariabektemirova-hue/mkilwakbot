...
7. Топ-3 на сегодня.
ВАЖНО:
- Не предлагай повторно этап процесса со статусом "готово".
- Если письмо Эльдара уже найдено, не ставь "найти письмо"; следующий шаг — включить МШР в повестку.
- Не требуй адрес или Zoom у каждой встречи автоматически.
- Давний хвост оригиналов с января не ставь в ежедневный Топ-3 без отдельной контрольной точки.
Будь компактной и практичной. Не выдумывай.
"""

    extra = "\n\n".join([cal, task_context, proc, control, mail_brief, prompt])
