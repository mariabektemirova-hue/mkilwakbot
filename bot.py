...
MODEL = "gpt-5.6"
BOT_VERSION = "v7.11.1-db-lock-fix"
...
def get_db():
    # SQLite is shared by the Telegram loop, reminder scan and reliable-memory helpers.
    # WAL lets readers coexist with a writer; busy_timeout makes short write collisions
    # wait briefly instead of failing immediately with "database is locked".
...
def update_task_verified(chat_id, task_id, *, text=None, due_at=None, notes=None, priority=None,
    ...
    log_work_event(chat_id,"task",task_id,"updated",raw_text,source_name,{
...
def update_control_verified(chat_id, control_id, *, next_action=None, due_at=None, notes=None, priority=None,
    ...
    log_work_event(chat_id,"control",control_id,"updated",raw_text,source_name,{
...
def apply_reconciled_work_input(chat_id, raw_text, *, source="natural", source_ref=None, source_name="",
    ...
    if action in {"update","complete"} and candidate and conf >= 0.55:
        ...
        note=str(decision.get("notes") or notes_hint or raw_text).strip()
...
def format_reconciled_result(result, source_name=""):
    ...
    if action=="update":
        ...
        label={"task":"задачу","control":"контрольное дело","process_step":"этап процесса"}.get(c.get("entity_type"),"дело")
        extra=""
        row=result.get("row")
        try:
            ...
            pass
        return f"Обновила существующее {label} ✅\n{c.get('title','')}" + extra
    if action=="complete":
        c=result.get("candidate") or {}
...
def repair_known_reliable_memory_artifacts(chat_id):
    """Safe cleanup of known pre-v7.10 duplicates.

    Important: never keep one SQLite connection open while calling helpers that
    open another connection and write. That nested pattern caused the v7.11.0
    "database is locked" failures. All candidate rows are materialized first,
    then the connection is closed before any verified writer is called.
    """
    try:
        ...
        if retro:
            conn = get_db()
            rows = conn.execute(
                "SELECT * FROM tasks WHERE chat_id=? AND status='open' ORDER BY id",
                (chat_id,),
            ).fetchall()
            conn.close()
            ...
            for t in rows:
                ...
                if "ретро" in n and "аналитичес" in n and ("папк" in n or "справк" in n or "записк" in n):
                    update_control_verified(
                        chat_id, retro["id"], next_action=t["text"],
                        notes="Слито из старой дублирующей задачи до v7.10.",
                        raw_text=t["text"],
                    )
                    merge_ids.append(t["id"])

            if merge_ids:
                conn = get_db()
                conn.executemany(
                    "UPDATE tasks SET status='merged',completed_at=NULL WHERE id=?",
                    [(task_id,) for task_id in merge_ids],
                )
                conn.commit()
                conn.close()
        ...
        if nikita and orphan and nikita["id"] != orphan["id"]:
            update_task_verified(
                chat_id, nikita["id"], due_at=orphan["due_at"],
                notes=orphan["text"] + " (слито из старой дублирующей задачи)",
                raw_text=orphan["text"],
            )
            conn = get_db()
            conn.execute(
                "UPDATE tasks SET status='merged',completed_at=NULL WHERE id=?",
        ...
        try:
            ...
            for item in parsed.get("items", [])[:6]:
                ...
                result = apply_reconciled_work_input(
                    ...
                    due_override=item.get("due_at") or None, notes_hint=str(item.get("notes") or "")
                )
                if result.get("action") == "create":
...
def try_handle_natural_work_request(chat_id, text):
    ...
    work_signals=[
        "надо","нужно","не забыть","попросила","попросил","сделаю","можно сделать","дожму","проверю","напишу",
        "поставить встречу","организовать встречу","прислала","прислал","внесла","внес","поставила","отправила",
        "готово","сделано","закрыла","во вторник","завтра надо","сегодня надо","перенесу","тогда завтра"
    ]
    if not any(s in n for s in work_signals):
        return False

    try:
        result=apply_reconciled_work_input(chat_id,raw,source="natural_work_request",source_name="",allow_create=True)
        if result.get("action") in {"create","update","complete"}:
            send_message(chat_id,format_reconciled_result(result))
            # For newly created meeting tasks, still ask for missing calendar coordinates.
            if result.get("action")=="create" and "встреч" in n:
                has_date=bool(resolve_due_from_text(raw) or re.search(r"\d{1,2}[./-]\d{1,2}|\d{1,2}\s+(?:августа|сентября|октября|ноября|декабря|января|февраля|марта|апреля|мая|июня|июля)",n))
                has_time=bool(re.search(r"\b\d{1,2}:\d{2}\b",n))
                if not has_date or not has_time:
                    send_message(chat_id,"Для календаря пока не хватает " + ("даты и времени." if not has_date and not has_time else "даты." if not has_date else "времени."))
            return True
    except Exception as e:
...
def main():
    init_db()
    print(f"Masha 2.0 {BOT_VERSION} запущена: reliable write + unified work resolver + conversational memory.")
    offset = None
    known_chat_ids = set()
    ...
    while True:
        ...
        try:
            ...
            for update in data.get("result", []):
                ...
                first_seen = chat_id not in known_chat_ids
