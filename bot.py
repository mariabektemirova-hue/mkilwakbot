...
MODEL = "gpt-5.6"
BOT_VERSION = "v7.9.4-closeout-ms-summary"
...
def apply_known_gift_update(chat_id, text):
    ...
    if "анжелик" in n and ("подпис" in n or "соглас" in n) and "приказ" in n:
        ...
        step_no = 4
    elif ("перв" in n or "№1" in text) and "акт" in n and (
        "получ" in n or "подпис" in n or "готов" in n
    ):
        step_no = 5
    elif ("перв" in n or "№1" in text) and ("счет" in n or "счёт" in text.lower()) and (
        "получ" in n or "есть" in n or "готов" in n
    ):
        step_no = 6
    elif ("втор" in n or "№2" in text) and "акт" in n and (
        "получ" in n or "подпис" in n or "готов" in n
    ):
        step_no = 7
    elif ("втор" in n or "№2" in text) and ("счет" in n or "счёт" in text.lower()) and (
        "получ" in n or "есть" in n or "готов" in n
    ):
        step_no = 8
...
- ask_ms = что нужно сказать/спросить/согласовать с МС;
...
def handle_closeout(chat_id, raw_text):
    ...
    for item in parsed.get("items", [])[:30]:
        ...
        if kind == "task":
            tid = add_task(chat_id, body, source="closeout", due_at=due, notes=str(item.get("notes") or ""))
            tasks.append((tid, body))
            ...
            if row:
                ...
                tid = add_task(chat_id, body, source="closeout", due_at=due)
                tasks.append((tid, body))
            ...
            if row:
                ...
                # Keep a standalone waiting action searchable.
                tid = add_task(
                    chat_id, body, source="closeout_waiting", due_at=due,
                    notes=f"Ждём: {item.get('waiting_for') or 'другого человека'}"
                )
                tasks.append((tid, body))

        elif kind == "done":
            # Record factual completion without inventing a process-step match.
            tid = add_task(chat_id, body, source="closeout_done", notes="Зафиксировано из итогов дня.")
            complete_task(chat_id, tid)
    ...
    if "один акт" in n and "подар" in n:
        c = "Полученный акт по подаркам — от художника №1 или художника №2?"
        if not any("художник" in normalize_text(x) and "акт" in normalize_text(x) for x in clarifications):
...
def build_ms_end_of_day_message(chat_id):
    """
    Build a copy-ready message TO MS only when MS actually needs to know,
    answer, approve, or do something today.
    This is separate from the internal /evening status.
    """
    ensure_alena_operations_question(chat_id)
    questions = get_open_ms_questions(chat_id)

    # Only include still-open MS questions/asks.
    actionable = []
    today = datetime.now(TZ).date()

    for q in questions:
        due_today_or_overdue = False
        if q["due_at"]:
            try:
                due_today_or_overdue = date.fromisoformat(q["due_at"]) <= today
            except Exception:
    ...
    lines = ["МС, на сегодня от тебя нужно:"]
    for q in actionable:
        text_value = q["text"].strip()
        # Make the wording cleaner for direct sending.
        n = normalize_text(text_value)
        if "вечернем апдейте" in n or "вечерний апдейт" in n:
            text_value = re.sub(
                r"(?i)^сегодня\s+в\s+вечернем\s+апдейте\s+надо\s+(спросить|уточнить|сказать)\s+у?\s*мс[,:]?\s*",
                "",
                text_value,
            ).strip()
            text_value = re.sub(
                r"(?i)^надо\s+(спросить|уточнить|сказать)\s+у?\s*мс[,:]?\s*",
                "",
                text_value,
            ).strip()
...
def show_help(chat_id):
    send_message(
        chat_id,
        """Masha 2.0 — шпаргалка 1/3

🧭 ГЛАВНОЕ
/brief — бриф на сегодня: календарь + задачи + процессы + контроль + почта
/control — все долгие/фоновые дела и даты контроля
/week — рабочий контроль на ближайшие 7 дней
/evening или /update — внутренний вечерний итог + вопросы МС + ожидания + следующие шаги
/msupdate — готовое короткое сообщение МС, только если от неё что-то нужно сегодня
/closeout — начать режим «подбить день»; следующим сообщением можно прислать весь brain dump
/questions — открытые вопросы/согласования с МС
/inbox — последние пересланные сообщения
/version — текущая версия бота
/health — проверка, что бот жив

Можно без /closeout написать:
«Подбиваю день: ...»
и дальше весь список одним сообщением."""
    )

    send_message(
        chat_id,
        """Masha 2.0 — шпаргалка 2/3

📅 КАЛЕНДАРЬ
/today — встречи сегодня
/tomorrow — встречи завтра
/calendarweek — календарь на 7 дней
/checktoday — проверить сегодняшний календарь: конфликты, воздух, повестки, подтверждения
/checkweek — проверить загрузку недели
Важно: календарь пока read-only — бот анализирует, но сам события не создаёт/не редактирует.

✅ ЗАДАЧИ
/tasks — открытые задачи
Задача: <текст> — записать задачу
/done 12 — закрыть задачу №12

🔁 ПРОЦЕССЫ
/processes — открытые процессы
/process 1 — процесс №1 подробно
/giftcase — процесс подарков директорам
/donep 1.5 — закрыть этап 5 процесса №1
Создай процесс: <название>
/step 1 <текст> — добавить этап в процесс №1"""
    )

    send_message(
        chat_id,
        """Masha 2.0 — шпаргалка 3/3

📨 ПОЧТА
/mail — проверить связь с Яндекс.Почтой
/mailrecent — последние письма
/mailcontacts — сохранённые проверенные контакты
Можно писать: «Найди письмо Эльдара», «Найди переписку с ...»
Почта пока read-only.

🧠 ПАМЯТЬ
/memory — постоянная память
Запомни: <текст> — сохранить факт
Забудь #12 — удалить запись памяти
/new — новый ИИ-диалог без удаления задач/памяти/процессов

👤 МС И РЕШЕНИЯ
Можно написать обычным языком:
«Сегодня в вечернем апдейте надо спросить у МС ...»
→ попадёт в /questions.

После реального ответа:
«МС ответила на Q1: ...»
→ вопрос закрывается, решение сохраняется; если нужен следующий шаг, бот создаст задачу.

↪️ ПЕРЕСЫЛКИ
Просто пересылай сообщения МС боту.
Очевидные поручения становятся задачами; сложные сообщения и скриншоты бот разбирает с контекстом."""
    )
...
def try_handle_command(chat_id, text):
    ...
    if clean in {"/msupdate", "/msbrief", "сообщение для мс", "апдейт для мс"}:
...
def main():
    init_db()
    print(f"Masha 2.0 {BOT_VERSION} запущена: fixed MS question context + closeout + ready-to-send MS end-of-day message.")
    offset = None
    known_chat_ids = set()
