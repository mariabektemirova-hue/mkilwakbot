...
def build_brief(chat_id):
    ...
    # Каждый источник собираем отдельно. Процессы — главный источник истины.
    proc = processes_context(chat_id)
    control = control_context(chat_id, due_only=True)

    tasks = get_open_tasks(chat_id)
    task_context = "ОБЫЧНЫЕ ЗАДАЧИ (НИЖЕ ПО ПРИОРИТЕТУ, ЧЕМ ПРОЦЕССЫ):\n" + (
        "\n".join(f"- #{r['id']}: {r['text']}" for r in tasks)
        if tasks else "Открытых задач нет."
    )
    ...
    try:
        ...
        mail_lines = ["СВЕЖАЯ ПОЧТА (ЗАГОЛОВКИ НЕ МЕНЯЮТ СТАТУС ПРОЦЕССОВ):"]
        if recent:
            for m in recent:
                mail_lines.append(
...
2. Риски календаря — реальные пересечения, отсутствие воздуха, явно отсутствующая повестка,
   неподтверждённые участники и действительно нужные для действия адреса/ссылки.
3. Что требует действия — только реально доступные сейчас действия.
4. Что ждём от других.
5. Что нужно проконтролировать.
6. Почта — только потенциально важное; по одному заголовку не придумывай содержание.
7. Топ-3 на сегодня.

ОБЯЗАТЕЛЬНАЯ ПРОВЕРКА ПЕРЕД ОТВЕТОМ:
- Сначала прочитай АКТУАЛЬНЫЕ ПРОЦЕССЫ.
- Все этапы [готово] исключи из "требует действия", "ждём" и Топ-3.
- Все этапы [заблокирован] не выдавай как действие на сейчас.
- Если старая обычная задача говорит "найти/получить/подписать", а процесс уже показывает
  соответствующий результат как [готово], старая задача устарела и должна быть проигнорирована.
- Не превращай контрольный долгосрочный хвост в срочный приоритет без даты проверки сегодня.
- Не пиши, что письмо Эльдара надо найти, если актуальный процесс уже фиксирует, что оно найдено.
- Не пиши, что нужна подпись Алёны или счёт художника №1, если они отмечены [готово].

Будь компактной, практичной и опирайся только на переданные ниже данные.
"""

    # Порядок здесь тоже отражает приоритет источников.
    source_context = "\n\n".join([
        "=== 1. АКТУАЛЬНЫЕ ПРОЦЕССЫ — ГЛАВНЫЙ ИСТОЧНИК ИСТИНЫ ===\n" + proc,
        "=== 2. КОНТРОЛЬНЫЕ ДЕЛА ===\n" + control,
        "=== 3. ОБЫЧНЫЕ ЗАДАЧИ — МОГУТ БЫТЬ УСТАРЕВШИМИ ===\n" + task_context,
...
def main():
    init_db()
    print(f"Masha 2.0 {BOT_VERSION} запущена: control + week + calendarweek + smart brief.")
    offset = None
    known_chat_ids = set()
