...
MODEL = "gpt-5.6"
BOT_VERSION = "v7.12.0-multi-action-reliable"
...
def resolve_multi_work_actions_with_ai(chat_id, raw_text, source_name=""):
    ...
    candidates = get_unified_work_candidates(chat_id, limit=100)
    last_type, last_id = get_last_work_ref(chat_id)
    lines = []
    for c in candidates:
        lines.append(
...
Верни только JSON:
{{"actions":[
  {{"action":"create|update|complete","entity_type":"task|control|process_step|none","entity_id":0,
...
def apply_multi_work_input(chat_id, raw_text, *, source="natural", source_ref=None, source_name=""):
    ...
    for idx,a in enumerate(actions,1):
        try:
            ...
            candidates=get_unified_work_candidates(chat_id,limit=140)
    ...
    if done:
        ...
        for r in done:
            ...
            if action=="create":
                ...
                label={"task":"TASK","control":"CONTROL","process_step":"STEP"}.get(c.get("entity_type"),"ДЕЛО")
...
def try_handle_natural_work_request(chat_id, text):
    ...
    try:
        # v7.12: one message may mutate several independent work objects.
