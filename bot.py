...
MODEL = "gpt-5.6"
BOT_VERSION = "v7.9.6.1-simple-workflow-fix"
...
def try_handle_natural_work_request(chat_id, text):
    """
    Everyday input with no slash command.
    Examples:
      "МС попросила поставить встречу с Никитой"
      "Мне завтра надо написать Вере"
      "Надо отправить МС ссылку на форму"
    """
    raw = text.strip()
    n = normalize_text(raw)
    ...
    # Do not steal explicit MS-answer/question or closeout flows.
    protected = (
        n.startswith("мс ответила")
        or n.startswith("подбиваю день")
        or n.startswith("закрываю день")
        or n.startswith("итоги дня")
        or "вечернем апдейте надо спросить" in n
    )
    if protected:
        return False
    ...
    # Meeting requests.
    meeting_patterns = [
        r"(?:мс\s+)?попросил[аи]?\s+(?:меня\s+)?поставить\s+встречу\s+(.+)",
        r"(?:мне\s+)?надо\s+поставить\s+встречу\s+(.+)",
        r"поставить\s+встречу\s+(.+)",
        r"(?:мс\s+)?попросил[аи]?\s+(?:меня\s+)?организовать\s+встречу\s+(.+)",
    ]
    ...
    for pat in meeting_patterns:
        ...
        # Use original text tail where possible.
        words = m.group(1).strip()
        task_text = f"Поставить встречу {words}".strip()
        tid = add_task(
            chat_id,
            task_text,
            source="natural_work_request",
            notes="Создано из обычного сообщения без команды.",
        )

        has_date = bool(re.search(
            r"\b(?:сегодня|завтра|послезавтра|понедельник|вторник|сред[ау]|четверг|пятниц[ау]|суббот[ау]|воскресень[ея]|\d{1,2}[./-]\d{1,2}|\d{1,2}\s+(?:августа|сентября|октября|ноября|декабря|января|февраля|марта|апреля|мая|июня|июля))\b",
            n,
            flags=re.I,
        ))
        has_time = bool(re.search(r"\b\d{1,2}:\d{2}\b", n))

        lines = [f"Записала ✅ #{tid} — {task_text}"]
        if not has_date and not has_time:
            lines.append("Для постановки в календарь не хватает даты и времени. Когда встреча?")
        elif not has_date:
            lines.append("Время вижу, но не хватает даты.")
        elif not has_time:
            lines.append("Дату вижу, но не хватает времени.")

        send_message(chat_id, "\n".join(lines))
        return True

    # Other obvious work requests.
    m = re.match(
        r"^(?:мс\s+попросил[аи]?\s+|мне\s+(?:сегодня\s+|завтра\s+)?надо\s+|надо\s+)(.+)$",
        raw,
        flags=re.I | re.S,
    )
    if m:
        ...
        if body and not body.endswith("?"):
            task_text = body[0].upper() + body[1:]
            tid = add_task(chat_id, task_text, source="natural_work_request")
            send_message(chat_id, f"Записала задачу ✅ #{tid} — {task_text}")
            return True
...
def main():
    init_db()
    print(f"Masha 2.0 {BOT_VERSION} запущена: fixed missing natural-work helpers + commands before natural language.")
    offset = None
    known_chat_ids = set()
