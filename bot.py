...
MODEL = "gpt-5.6"
BOT_VERSION = "v7.7-evening-decisions"
...
def recent_forward_context(chat_id, limit=8):
    conn = get_db()
    rows = conn.execute(
        """
        SELECT source_name,raw_text,created_at
    ...
    if not rows:
        return "Недавних пересылок нет."

    lines = ["НЕДАВНИЕ ПЕРЕСЫЛКИ (только для понимания контекста):"]
    for row in reversed(rows):
        lines.append(
            f"- {row['source_name'] or 'источник не указан'}: "
            f"{(row['raw_text'] or '[изображение]')[:700]}"
        )
    return "\n".join(lines)
...
def build_evening_update(chat_id):
    ...
    # Questions are rendered by code, not by AI. Therefore Q1/Q2 can never be omitted.
    lines = ["Итоги дня"]
    ...
    # AI only prepares the internal continuation block from authoritative current state.
    source_context = "\n\n".join([
        brief_processes_context(chat_id),
        control_context(chat_id, due_only=True),
        brief_tasks_context(chat_id),
    ])
    prompt = f"""
Составь ТОЛЬКО продолжение вечернего рабочего итога Маши после блока вопросов МС.
Не повторяй вопросы МС и не создавай раздел "Вопросы" — он уже сформирован программой.

Нужны максимум три коротких раздела:
1) "Жду от других:" — только реально открытые ожидания.
2) "Следующие шаги:" — 3–5 реально доступных действий, не заблокированных зависимостями.
3) "Несрочные хвосты:" — только если сегодня наступила контрольная дата или хвост действительно стоит подсветить.

Жёстко:
- не возвращай закрытые этапы;
- не предлагай заблокированные этапы;
- не делай давние оригиналы ежедневным топ-приоритетом без контрольной даты;
- по Эльдару письмо уже найдено: следующий шаг — повестка МШР;
- не придумывай сроки и выполненные действия.

ДАННЫЕ:
{source_context}
"""
    try:
        tail = ask_openai_grounded_brief(chat_id, prompt).strip()
        if tail:
            lines += ["", tail]
    except Exception as e:
        print("Evening tail error:", repr(e))
        lines += ["", "Не смогла собрать блок следующих шагов, но вопросы МС выше сохранены полностью."]
...
def main():
    init_db()
    print(f"Masha 2.0 {BOT_VERSION} запущена: guaranteed evening questions + MS decisions + task dedupe.")
    offset = None
    known_chat_ids = set()
