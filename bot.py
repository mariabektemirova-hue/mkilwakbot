...
def try_handle_command(chat_id, text):
    ...
    if clean == "/closeout":
        send_message(
            chat_id,
            "Напиши следующим сообщением начиная с «Подбиваю день:» и дальше всё одним текстом — что сделано, что ждёшь, что завтра и что спросить у МС."
        )
        return True
    if re.match(r"^(?:подбиваю день|закрываю день)\s*[:：]", normalized, flags=re.I):
        body = re.sub(r"^(?:подбиваю день|закрываю день)\s*[:：]\s*", "", normalized, flags=re.I)
...
def main():
    init_db()
    print(f"Masha 2.0 {BOT_VERSION} запущена: evening brain dump to tasks + control + MS questions.")
    offset = None
    known_chat_ids = set()
