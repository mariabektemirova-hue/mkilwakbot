...
MODEL = "gpt-5.6"
BOT_VERSION = "v7.9.2-help-statefix"
...
{{
 "items":[{{
   "kind":"task|ask_ms|control_update|done|waiting|clarification|info",
   "text":"самодостаточная формулировка",
   "due_at":"YYYY-MM-DD или null",
   "waiting_for":"кто или null",
   "control_hint":"какое существующее контрольное дело или null",
   "notes":""
 }}]
}}
"""
    payload = {
        "model": MODEL,
        "instructions": "Верни только валидный JSON без Markdown.",
        "input": prompt,
        "store": False,
    }
    r = requests.post(
    ...
    r.raise_for_status()
    obj = parse_json_from_model(extract_openai_text(r.json()))
    if not isinstance(obj, dict) or not isinstance(obj.get("items"), list):
        raise ValueError("invalid closeout JSON")
    return obj
...
def handle_closeout(chat_id, raw_text):
    ...
    try:
        ...
        print("Closeout parse error:", repr(e))
        send_message(chat_id, "Не смогла разобрать итоги. Ничего не меняла. Посмотри Closeout parse error в Railway.")
        return True
    ...
    lines += ["", "После уточнений можно вызвать /evening — он соберёт апдейт уже из обновлённого состояния."]
    send_message(chat_id, "\n".join(lines))
    return True
...
/evening или /update — вечерний итог + вопросы МС + ожидания + следующие шаги
...
def main():
    init_db()
    print(f"Masha 2.0 {BOT_VERSION} запущена: conversation_state fixed + /help.")
    offset = None
    known_chat_ids = set()
